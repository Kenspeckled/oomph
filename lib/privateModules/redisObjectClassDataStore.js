...
(function() {
  ...
  redisObjectDataStore = {
    ...
    where: function(args) {
      var attr, end, field, fields, integerSortedSetName, j, k, keyword, keywordSearchPromise, keywords, l, len, len1, len2, len3, len4, m, maxValue, minValue, modifyObj, modifyObjAttrs, n, option, optionValue, prepareWhereConditionPromise, ref, ref1, referenceModelName, self, sortedSetKeys, start, tempIntegerKey, unionSortedSetKeys, weight, weightOptions, whereConditionPromises;
      self = this;
      args || (args = {});
              ...
              if (this.attributes[option].many) {
                if (optionValue.includesAllOf) {
                  _.each(optionValue.includesAllOf, function(id) {
                    return sortedSetKeys.push({
                      name: referenceModelName + ':' + id + '#' + self.name + 'Refs'
                    });
                  });
                }
                ...
                if (optionValue.anyOf) {
                  ...
                  sortedSetKeys.push({
                    name: referenceModelName + ':' + optionValue + '#' + self.name + 'Refs'
                  });
                }
              }
            }
