        ...
        switch (obj.dataType) {
          ...
          case 'reference':
            if (obj.many) {
              ...
              multi.sadd.apply(multi, [self.name + ":" + props.id + "#" + obj.referenceModelName + 'Refs'].concat(slice.call(multipleValues)));
              multipleValues.forEach(function(vid) {
                return multi.sadd(obj.referenceModelName + ":" + vid + "#" + self.name + 'Refs', props.id);
              });
            }
            break;
          default:
  ...
  redisObjectDataStore = {
    ...
    find: function(id) {
      ...
      modifyHashPromise = getHash.then(function(hash) {
        var attrSettings, getReferenceIds, hashPromise, j, len, propertyName, propertyValue, ref, referencePromise;
        ref = Object.keys(hash);
        for (j = 0, len = ref.length; j < len; j++) {
          propertyName = ref[j];
          propertyValue = hash[propertyName];
          attrSettings = self.attributes[propertyName];
          if (attrSettings && attrSettings.dataType === 'reference') {
            if (attrSettings.many) {
              getReferenceIds = new Promise(function(resolve, reject) {
                var hashObj, referenceKey;
                hashObj = {
          ...
          if (newValue !== originalValue || _.includes([true, 'true', false, 'false'], newValue) || removeValue) {
            ...
            switch (obj.dataType) {
              ...
              case 'reference':
                if (obj.many) {
                  if (remove) {
                    multi.srem.apply(multi, [self.name + ":" + id + "#" + obj.referenceModelName + 'Refs'].concat(slice.call(removeValue)));
                    removeValue.forEach(function(vid) {
                      return multi.srem(obj.referenceModelName + ":" + vid + "#" + self.name + 'Refs', id);
                    });
                  } else {
