...
(function() {
  ...
  createObject = function(newObjectProps) {
    var key, obj, props, value;
    props = {};
    obj = Object.create(this.prototype);
    if (newObjectProps.id) {
      obj.createdAt = idToCreatedAtDate(newObjectProps.id);
    }
    for (key in newObjectProps) {
      value = newObjectProps[key];
  ...
  indexSearchableString = function(attr, words, id) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      for (k = 0, len1 = word.length; k < len1; k++) {
        char = word[k];
        wordSegment += char;
        wordSegmentKey = this.name + '#words:' + attr + ':' + wordSegment;
  ...
  writeAttributes = function(props) {
    var idPromise, indexPromise, self, writePromise;
    self = this;
    ...
    writePromise = idPromise.then(function(id) {
      ...
      return new Promise(function(resolve) {
        return self.redis.hmset(objectIdentifier, props, function(err, res) {
          return resolve();
        });
      });
    });
    indexPromise = writePromise.then(function() {
      var attr, indexPromiseFn, indexingPromises, multi, multipleValues, obj, ref, sortedSetName, value;
      indexingPromises = [];
        ...
        switch (obj['dataType']) {
          ...
          case 'association':
            if (obj.many) {
              ...
              multi.sadd.apply(multi, [self.name + "#" + attr + ":" + props.id].concat(slice.call(multipleValues)));
            }
            break;
          default:
  ...
  addToWriteQueue = function(props) {
    ...
    return p.then(function(tmpId) {
      ...
      return new Promise(function(resolve) {
        return publishSubscribe.listen.apply(self, [
          "attributes_written_" + tmpId, function(obj) {
            return resolve(obj);
          }
        ]);
      });
    ...
    unionKeyPromises = _.map(keywords, (function(_this) {
      return function(keyword) {
        ...
        for (j = 0, len = fields.length; j < len; j++) {
          ...
          keyNames.push({
            name: _this.name + "#words:" + field + ":" + keyword,
            weight: weight
          });
        }
  ...
  redisObjectDataStore = {
    ...
    find: function(id) {
      var findAssociationPromiseObjects, foundPromise;
      findAssociationPromiseObjects = [];
      foundPromise = new Promise((function(_this) {
        return function(resolve, reject) {
          return _this.redis.hgetall(_this.name + ":" + id, function(error, props) {
            var j, len, obj, propertyName, propertyValue, ref;
            if (props) {
              ref = Object.keys(props);
              for (j = 0, len = ref.length; j < len; j++) {
                propertyName = ref[j];
                propertyValue = props[propertyName];
                if (_this.attributes[propertyName]) {
                  switch (_this.attributes[propertyName].dataType) {
                    case 'integer':
                      props[propertyName] = parseInt(propertyValue);
                      break;
                    case 'boolean':
                      if (_.includes([true, 'true'], propertyValue)) {
                        props[propertyName] = true;
                      }
                      if (_.includes([false, 'false'], propertyValue)) {
                        props[propertyName] = false;
                      }
                      break;
                    case 'association':
                      propertyValue = propertyValue.split(',');
                      if (_this.attributes[propertyName].many) {
                        props[propertyName] = propertyValue;
                      } else {
                        props[propertyName] = propertyValue[0];
                      }
                      if (_this.attributes[propertyName].preloadModel) {
                        _.each(propertyValue, function(v) {
                          var associationPromise;
                          associationPromise = new Promise(function(res) {
                            var Model;
                            Model = _this.attributes[propertyName].preloadModel;
                            return Model.find(v).then(function(foundObj) {
                              return res(foundObj);
                            });
                          });
                          return findAssociationPromiseObjects.push({
                            propertyName: propertyName,
                            promise: associationPromise,
                            many: _this.attributes[propertyName].many
                          });
                        });
                      }
                  }
              ...
              obj = createObject.apply(_this, [props]);
              return resolve(obj);
            } else {
      ...
      return foundPromise.then(function(foundObj) {
        var assignAssociationsPromises;
        if (_.isEmpty(findAssociationPromiseObjects)) {
          return foundObj;
        }
        _.each(findAssociationPromiseObjects, function(associationPromiseObj) {
          if (associationPromiseObj.many) {
            return foundObj[associationPromiseObj.propertyName] = [];
          } else {
            return foundObj[associationPromiseObj.propertyName] = null;
          }
        });
        assignAssociationsPromises = _.map(findAssociationPromiseObjects, function(associationPromiseObj) {
          return associationPromiseObj.promise.then(function(associationObj) {
            if (associationPromiseObj.many) {
              if (associationObj) {
                return foundObj[associationPromiseObj.propertyName].push(associationObj);
              }
            } else {
              return foundObj[associationPromiseObj.propertyName] = associationObj;
            }
          });
        });
        return Promise.all(assignAssociationsPromises).then(function() {
          return foundObj;
        });
      });
    },
    ...
    where: function(args) {
      var assocationClass, attr, end, field, fields, integerSortedSetName, j, k, keyword, keywordSearchPromise, keywords, l, len, len1, len2, len3, len4, m, manySetName, maxValue, minValue, modifyObj, modifyObjAttrs, n, option, optionValue, pluralAttr, prepareWhereConditionPromise, ref, ref1, self, sortedSetKeys, start, tempIntegerKey, unionSortedSetKeys, weight, weightOptions, whereConditionPromise;
      self = this;
      args || (args = {});
      ...
      if (args.sortBy === 'rand') {
        sortedSetKeys.push({
          name: this.name + 'OrderedSet#ids'
        });
      } else if (args.sortBy !== 'relevance') {
        sortedSetKeys.push({
          name: this.name + 'OrderedSet#' + pluralise(args.sortBy)
        });
      }
      weightOptions = {};
        ...
        if (args.includes.inAnyOf) {
          ...
          for (l = 0, len2 = fields.length; l < len2; l++) {
            ...
            for (m = 0, len3 = keywords.length; m < len3; m++) {
              keyword = keywords[m];
              sortedSetKeys.push({
                name: this.name + "#words:" + field + ":" + keyword,
                weight: weight
              });
            }
          }
      ...
      for (n = 0, len4 = ref1.length; n < len4; n++) {
        ...
        switch (this.attributes[option].dataType) {
          case 'integer':
            ...
            integerSortedSetName = this.name + "#" + pluralise(option);
            minValue = '-inf';
            maxValue = '+inf';
            ...
            whereConditionPromise = new Promise(function(resolve) {
          ...
          case 'boolean':
            ...
            break;
          case 'association':
            assocationClass = this.attributes[option].preloadModel;
            if (assocationClass) {
              pluralAttr = pluralise(self.name.toLowerCase());
              if (assocationClass.getName().toLowerCase() === option) {
                if (assocationClass.attributes[pluralAttr] && assocationClass.attributes[pluralAttr].many) {
      ...
      if (!whereConditionPromise) {
        whereConditionPromise = new Promise(function(r) {
          return r();
        });
      }
      prepareWhereConditionPromise = whereConditionPromise.then(function() {
      ...
      return prepareWhereConditionPromise.then(function() {
        ...
        idsKeyPromise = new Promise(function(resolve) {
          ...
          return (ref2 = self.redis).zinterstore.apply(ref2, [intersectKey, sortedSetKeys.length].concat(slice.call(sortedSetKeyNames), [function(err, numberOfResults) {
            self.redis.expire(intersectKey, 1);
            return resolve({
    ...
    update: function(id, updateFields, skipValidation) {
      var callbackPromises, getOriginalObjPromise, multi, updateFieldsDiff;
      updateFieldsDiff = {
        id: id
      };
      callbackPromises = [];
      multi = this.redis.multi();
      getOriginalObjPromise = this.find(id).then(function(originalObj) {
        if (!originalObj) {
          throw new Error("Id not found");
        }
        return originalObj;
      });
      return getOriginalObjPromise.then((function(_this) {
        return function(originalObj) {
          var attr, j, len, multiPromise, newValue, obj, orderedSetName, originalIds, originalValue, ref, remove, removeValue, sortedSetName;
          ...
          for (j = 0, len = ref.length; j < len; j++) {
            ...
            orderedSetName = _this.name + "OrderedSet" + "#" + pluralise(attr);
            originalValue = originalObj[attr];
            newValue = updateFields[attr];
            if (newValue !== originalValue || _.includes([true, 'true', false, 'false'], newValue)) {
              updateFieldsDiff[attr] = newValue;
              if (remove) {
                delete updateFieldsDiff[attr];
              }
              obj = _this.attributes[attr];
              if (!obj) {
                return;
              }
              switch (obj.dataType) {
                case 'integer':
                  sortedSetName = _this.name + "#" + pluralise(attr);
                  multi.zrem(sortedSetName, id);
                  break;
                case 'text':
                  if (obj.searchable) {
                    callbackPromises.push(removeIndexedSearchableString.apply(_this, [attr, originalValue, id]));
                  }
                  break;
                case 'string':
                  if (obj.sortable) {
                  ...
                  if (obj.identifiable) {
                    multi.del(_this.name + "#" + attr + ":" + originalValue);
                  }
                  break;
                case 'association':
                  if (obj.many === true) {
                    if (remove === true) {
                      multi.srem.apply(multi, [_this.name + "#" + attr + ":" + id].concat(slice.call(removeValue)));
                    } else {
                  ...
                  break;
                case 'boolean':
                  multi.zrem(_this.name + "#" + attr + ":" + originalValue, id);
              }
            }
          }
          multiPromise = new Promise(function(resolve, reject) {
            return multi.exec(function() {
              return sendAttributesForSaving.apply(_this, [updateFieldsDiff, skipValidation]).then(function(writtenObj) {
                return resolve(_this.find(writtenObj.id));
              }, function(error) {
                return reject(error);
              });
            });
